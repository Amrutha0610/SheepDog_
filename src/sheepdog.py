#Importing essential libraries: PyTorch, Transformers, and others
import torch
...
import numpy as np
import sys,os
#Include project directory in path to import utils
...
#Parses command-line arguments for dataset, training options, and filtering toggle
...
parser.add_argument('--use_small_dataset', action='store_true', help='Use small dataset subset')
parser.add_argument('--use_filtering', action='store_true', help='Apply content filtering to remove stylistic noise')
parser.add_argument('--max_len', default=512, type=int, help='Maximum sequence length for tokenization')

args = parser.parse_args()
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
#Ensures reproducibility by fixing seeds
torch.manual_seed(0)
np.random.seed(0)
torch.backends.cudnn.deterministic = True
torch.cuda.manual_seed_all(0)
        
#Dataset for training with augmented (style-reframed) texts and fine-grained labels
...
#Dataset for standard evaluation (no augmentation)
...
#RoBERTa-based classifier with two outputs:
#-one for fine-grained label prediction (multi-label)
...
def create_train_loader(contents, contents_aug1, contents_aug2, labels, fg_label, aug_fg1, aug_fg2, tokenizer, max_len, batch_size):
    # Prepares training DataLoader from augmented data
...
def create_eval_loader(contents, labels, tokenizer, max_len, batch_size):
    # Prepares test DataLoader from standard text inputs
    ds = NewsDataset(texts = contents, labels = np.array(labels), tokenizer=tokenizer, max_len=max_len)
    
    return DataLoader(ds, batch_size=batch_size, num_workers=0, drop_last=True)
...
#----------------Dry run check-------------------------------
#Verifies pipeline integrity before full training
def dry_run_check(tokenizer, max_len, batch_size, datasetname):
    print("\n Running Dry Run Check...\n")

    from utils.load_data import load_articles

    x_train, x_test, x_test_res, y_train, y_test = load_articles(datasetname, use_filtering=True)
    x_test_res = x_test_res[:len(y_test)]  # match lengths

    loader = create_eval_loader(x_test_res, y_test, tokenizer, max_len, batch_size)

    for batch in loader:
        print("Batch loaded successfully! Shape:", batch['input_ids'].shape)
        break

    print("Dry Run Passed — ready for full training.\n")
#------------------------------Training model-------------------------------- 
#handles training loop, validation, and metrics
def train_model(tokenizer, max_len, n_epochs, batch_size, datasetname, iter):

    x_train, x_test, x_test_res, y_train, y_test = load_articles(datasetname, use_filtering=args.use_filtering)
    ...
    for epoch in range(n_epochs):
        ...
        #On last epoch, evaluate both standard and style-restyled test sets
        if epoch == n_epochs - 1:
            model.eval()
            y_pred = []
            y_pred_res = []
...
if args.use_filtering:
    dry_run_check(tokenizer, args.max_len, args.batch_size, args.dataset_name)

if __name__ == "__main__":
    import multiprocessing
    multiprocessing.freeze_support()  # For Windows multiprocessing support
